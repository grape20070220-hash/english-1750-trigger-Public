...
function remainingText(minutes: number | null) {
  if (minutes === null) return "残高を設定";
...
export default function ApiBudgetSummary({ onOpen }: { onOpen: () => void }) {
  ...
  const statusLabel = status === "empty" ? "残高切れ" : status === "critical" ? "残りわずか" : status === "low" ? "少なめ" : status === "ok" ? "利用可能" : "未設定";
  ...
  return (
    <div className="apiBudgetDock">
      ...
      `}</style>
      <button className={`apiBudgetQuick ${status}`} onClick={onOpen} aria-label="API残量とチャージを開く">
        <SparkIcon />
        <div className="apiBudgetCopy">
          <small>API 残量</small>
          <strong>{budget ? remainingText(budget.remainingMinutes) : "残量を確認中…"}{budget?.remainingUsd !== null && budget?.remainingUsd !== undefined ? ` ・ $${budget.remainingUsd.toFixed(2)}` : ""}</strong>
        </div>
        <div className="apiBudgetMeta"><span>{statusLabel}</span><b>詳細・チャージ →</b></div>
      </button>
    </div>
  );
}
